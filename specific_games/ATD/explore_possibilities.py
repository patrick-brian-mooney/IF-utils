...
This script is copyright 2019-25 by Patrick Mooney. It is released under the
...
from mod.terps_for_specific_games import ATDTerpConnection as ATD_tc
...
def only_in_prototype(*pargs) -> bool:
    """This function is a filter for actions that are only productive when the
...
def only_in(c: str, where: List[str]) -> bool:
...
class MetadataWriter(threading.Thread):
    ...
    def queue_for_write(self, data: Dict[str, Dict[str, Union[int, float]]]) -> None:
...
class ATDExplorerTerpConnection(ATD_tc):
    progress_checkpoint_file = ATD_tc.working_directory / 'progress.pkl'
    ...
    def evaluate_context(self, output: str,
                         command: str) -> Dict[str, Union[str, int, bool, Path, List[str]]]:
...
terp_proc: Optional[ATDExplorerTerpConnection] = None        # we'll reassign this during setup.
